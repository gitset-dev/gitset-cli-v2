...
function reconcileUnchangedEntries(moduleSummaries, affectedModuleNames, diff, previousModuleSummaries) {
  const reallyChanged = new Set([...diff.changed, ...diff.added]);
  const previous = previousModuleSummaries || {};
  for (const modName of affectedModuleNames) {
    const entries = moduleSummaries[modName];
    if (!Array.isArray(entries)) continue;
    const prevEntries = Array.isArray(previous[modName]) ? previous[modName] : [];
    const prevByPath = new Map(prevEntries.map((e) => [e.path, e]));
    moduleSummaries[modName] = entries.map((entry) => {
      if (reallyChanged.has(entry.path)) return entry;
      const prior = prevByPath.get(entry.path);
      return prior || entry;
    });
  }
