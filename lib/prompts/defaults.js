...
const clipCommits = (s, n) => {
  const str = String(s ?? '');
  if (str.length <= n) return str;
  const lines = str.split('\n');
  if (lines.length <= 1) return clip(str, n);
  ...
  while (i <= j && headLen + lines[i].length + 1 <= n / 2) {
    headLen += lines[i].length + 1;
    head.push(lines[i]);
    i += 1;
  }
  while (j >= i && tailLen + lines[j].length + 1 <= n / 2) {
  ...
  return [...head, `…(${omitted} more commits omitted here for length)…`, ...tail].join('\n');
};
  ...
  build(ctx = {}) {
    ...
    return {
      system:
        ...
        'If the commit list contains a line like "…(N more commits omitted here for ' +
        'length)…", that is NOT a commit — it is metadata for you. Never describe, ' +
        'quote, or bullet it as a change. Instead, summarize the visible commits as ' +
        'usual and add one closing line noting that N additional commits from this ' +
        'range were not included due to length.',
      user: [
...
const knowledgeWrite = {
  id: 'knowledgeWrite',
  build(ctx = {}) {
    const { repo = '', doc = '', sections = '', guidance = '', digest = '', summaries = '', previous = '' } = ctx;
    return {
      system:
        ...
        (previous
          ? ' A previous version of this document is included below the ground ' +
            'truth, because this is an incremental update, not a fresh rewrite. ' +
            'Preserve its exact wording, ordering, and phrasing for anything the ' +
            'ground truth still supports unchanged — do not rephrase or ' +
            'reorganize content just because you would have written it ' +
            'differently. Only add, remove, or edit the specific facts the ' +
            'ground truth shows have actually changed.'
          : ''),
      user: [
        repo && `Repository: ${clip(repo, 200)}`,
        ...
        previous && `Previous version of this document (preserve unaffected wording):\n${clip(previous, 20000)}`,
      ].filter(Boolean).join('\n\n'),
    };
  },
};
