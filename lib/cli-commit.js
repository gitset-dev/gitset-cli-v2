...
async function runCommitCommand(argv) {
  ...
  async function gen(previous, instruction) {
    try {
      const label = `generating via ${providerCfg.provider} (${baseCfg.model || 'default model'})`;
      const call = generateCommitMessage({
        providerCfg: baseCfg, diff: changes.diff, stats: changes.stat, style, previous, instruction,
      });
      return await ((!json && !printOnly) ? spinner.withSpinner(label, call) : call);
    } catch (e) {
